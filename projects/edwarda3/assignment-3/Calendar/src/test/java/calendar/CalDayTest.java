...
public class CalDayTest {
	...
	public void test03()  throws Throwable  {
		...
		assertEquals(appt3,cd.getAppts().get(1));

		CalDay sortthis = new CalDay(newyears);
		for(int i=23; i>=0; i--){
			Appt testAppt = new Appt(i,1,1,1,1,i+"th hour","test");
			sortthis.addAppt(testAppt);
			assertEquals(testAppt,sortthis.getAppts().get(0));
		}
		CalDay sortthis2 = new CalDay(newyears);
		for(int i=0; i<23; i++){
			Appt testAppt = new Appt(i,1,1,1,1,i+"th hour","test");
			sortthis2.addAppt(testAppt);
			assertEquals(testAppt,sortthis2.getAppts().get(sortthis2.getSizeAppts()-1));
		}
	}
